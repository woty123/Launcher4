...
public final class Launcher extends Activity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        System.out.println("wbzhang----onCreate invoked!----->>>>>>>>>");

        LauncherApplication app = ((LauncherApplication) getApplication());
        mSharedPrefs = getSharedPreferences(LauncherApplication.getSharedPreferencesKey(),
                Context.MODE_PRIVATE);
        // 得到Application中创建的LauncherModel的一个引用  将自己实现的LauncherModel.Callbacks引用到LauncherModel
        mModel = app.setLauncher(this);
        // 得到Application中创建的IconCache的一个引用
        mIconCache = app.getIconCache();
        mDragController = new DragController(this);
        mInflater = getLayoutInflater();

        mAppWidgetManager = AppWidgetManager.getInstance(this);
        // APPWIDGET_HOST_ID 相当于一个唯一的标识码，这个码只在当前程序有效，不必考虑与其它应用程序冲突
        ...
        setContentView(R.layout.launcher); // sw720
        setupViews();
        ...
        /*
         * if (!mRestoring) { mModel.startLoader(true); }
         */
    ...
    private void checkForLocaleChange() {
        if (sLocaleConfiguration == null) {
            // 创建一个异步线程，读取系统配置 LocaleConfiguration 为返回结果
            new AsyncTask<Void, Void, LocaleConfiguration>() {
                @Override
                protected LocaleConfiguration doInBackground(Void... unused) {
                    LocaleConfiguration localeConfiguration = new LocaleConfiguration();
                    // 这里读取到参数 返回到onPostExecute执行，sLocaleConfiguration中有了内容
                ...
                protected void onPostExecute(LocaleConfiguration result) {
                    ...
                    // 再次调用的时候sLocaleConfiguration 不为空
        ...
        if (localeChanged) {
            // sLocaleConfiguration 中的数据修改成当前系统的数据
            sLocaleConfiguration.locale = locale;
            sLocaleConfiguration.mcc = mcc;
            sLocaleConfiguration.mnc = mnc;
            // 当地区改变了，将保存程序的数据清空 因为配置的改变可能会改变语言环境
            mIconCache.flush();
    ...
    /**
     * 系统配置信息的一个内部类locale地理位置、mcc移动国家代码 mnc移动网络代码
     *
     * @author wbzhang
     */
    ...
    private void restoreState(Bundle savedState) {
        ...
        final long pendingAddContainer = savedState
                .getLong(RUNTIME_STATE_PENDING_ADD_CONTAINER, -1);
        ...
        if (mAppsCustomizeTabHost != null) {
            ...
            if (curTab != null) {
                // We set this directly so that there is no delay before the tab
                // is set
    ...
    private void setupViews() {
        ...
        // Setup the hotseat // 下在那五个快捷方式 平板没有
        ...
        // Setup the workspace
        mWorkspace.setHapticFeedbackEnabled(false); // 设置取消触摸反馈
    ...
    void completeAddApplication(Intent data, long container, int screen, int cellX, int cellY) {
        ...
        if (info != null) {
            ...
            mWorkspace.addApplicationShortcut(info, layout, container, screen, cellXY[0],
                    cellXY[1],
    ...
    private void completeAddAppWidget(final int appWidgetId, long container, int screen,
        ...
        launcherInfo.providerName = appWidgetInfo.provider;
    ...
    void startActivityForResultSafely(Intent intent, int requestCode) {
        try {
            ...
            if ("com.android.email.widget.WidgetConfiguration".equals(cmp.getClassName()))
            {
                Toast.makeText(this, R.string.email_activity_not_found, Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(this, R.string.activity_not_found, Toast.LENGTH_SHORT).show();
            }
        } catch (SecurityException e) {
    ...
    public boolean onLongClick(View v) {
        ...
        // 只要mWorkspace.allowLongPress返回true即返回true
    ...
    public boolean isAllAppsVisible() {
        System.out.println("wbzhang ----- mState:" + mState);
     ...
     * @param state The state that we are moving in or out of (eg.
     *            APPS_CUSTOMIZE)
     ...
     * loader in onResume. This needs to be called from incoming places where
     * resources might have been loaded while we are paused. That is becaues the
     * Configuration might be wrong when we're not running, and if it comes back
     * to what it was when we were paused, we are not restarted. Implementation
    ...
    public int getCurrentWorkspaceScreen() {
        if (mWorkspace != null) {
            // 第一次是在workspace创建时设置的mCurrentPage  也就是workspace中的mDefaultPage
    ...
    /**
     * 刷新workspace中的快捷方式显示 Refreshes the shortcuts shown on the workspace.
     * Implementation of the method from LauncherModel.Callbacks.
     */
     ...
     * 从shortcuts中绑定start到end个到workspace
     */
     ...
     *
     * @param immediate whether to run the animation or show the results
     *            immediately
     */
    ...
    public void bindAllApplications(final ArrayList<ApplicationInfo> apps) {
        ...
        View progressBar = mAppsCustomizeTabHost.
                findViewById(R.id.apps_customize_progress_bar);   // 720P is null
        if (progressBar != null) {
