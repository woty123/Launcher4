...
public class LauncherModel extends BroadcastReceiver {
    ...
    private int mBatchSize; // 0 is all apps at once　　　每次加载App的数量　0代表一次性加载全部
    ...
    // WeakReference Java弱引用  与Launcher通信
    ...
    // LauncherModel to their ids   所有的ItemInfo与Id的键值对
    ...
    // sAppWidgets is all LauncherAppWidgetInfo created by LauncherModel. Passed to bindAppWidget()  pass to 交给
    ...
    public void startLoader(boolean isLaunching) {
        synchronized (mLock) {
            ...
            if (mCallbacks != null && mCallbacks.get() != null) {
                ...
                // 当isLaunching 为false的时候 将会执行stopLoaderLocked()方法，如果为true  则短路不执行
    ...
    private class LoaderTask implements Runnable {
        ...
        private void loadAndBindWorkspace() {
            ...
            // Bind the workspace  数据传给到Launcher
            bindWorkspace();
        }
        ...
        private void waitForIdle() {
            ...
            synchronized (LoaderTask.this) {
                ...
                // 将一个Runnable加入到最后，因为是在最后，只有前面的Runnable都执行了，才会执行这个，用这个来确定LoaderTask执行完成与否
        ...
        public void run() {
            ...
            // 当第一次运行时cbk.isAllAppsVisible() 为false，它的State为WorkSpace而不是APPS_CUSTOMIZE
            final boolean loadWorkspaceFirst = cbk != null ? (!cbk.isAllAppsVisible()) : true;

            // Java 标号Label语法，用于break跳出，相当于一个标签
            keep_running: {
                ...
                // second step　　loadAndBindAllApps
                if (loadWorkspaceFirst) {
        ...
        private void loadWorkspace() {
            ...
            // LauncherProvider在程序创建的时候就设置到LauncherApplication中
            ...
            System.out.println("wbzhang ---------   before qurey");
            ...
            //代表屏幕中的每一个单位的方格是否被占用。
            //第一维表示分屏的序号，其中最后一个 ＋1 代表的是Hotseat
            //第二维表示x方向方格的序号
            //第三维表示y方向方格的序号
            ...
            try {
                ...
                long id;
                Intent intent;
                System.out.println("wbzhang  -------   c.Count:" + c.getCount());
                while (!mStopped && c.moveToNext()) {  // 遍历所有的内容
                    try {
                        // 首先获取类型
                        int itemType = c.getInt(itemTypeIndex);

                        switch (itemType) {
                            case LauncherSettings.Favorites.ITEM_TYPE_APPLICATION: // favorite的为这个类型
                            case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT: // TAG_SHORTCUT
                                                                                // 为这个类型
                                intentDescription = c.getString(intentIndex);
                                try {
                                    // 保存的时候是调用intent.toUri(flags)作为字符串保存的  现在还原回来
                                ...
                                if (info != null) {
                                    ...
                                    // check & update map of what's occupied  检查info所在地方是否被占用
                                    ...
                                    switch (container) {
                                        ...
                                        case LauncherSettings.Favorites.CONTAINER_HOTSEAT:
                                            sWorkspaceItems.add(info);  // 加入到sWorkspaceItems
                                            break;
                                        default:
                                    ...
                                    queueIconToBeChecked(sDbIconCache, info, c, iconIndex);  // 将info的Icon信息放入sDbIconCache
                                } else {
                                    ...
                                    contentResolver.delete(
                                            LauncherSettings.Favorites.getContentUri(
                                                    id, false), null, null);  // 找不到这个shortcut通过URI删除
                                }
                                break;
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_FOLDER:
                                ...
                                switch (container) {
                                    case LauncherSettings.Favorites.CONTAINER_DESKTOP:
                                    case LauncherSettings.Favorites.CONTAINER_HOTSEAT:
                                        sWorkspaceItems.add(folderInfo);  // folderInfo也加入到sWorkspaceItems
                                        break;
                                }
        ...
        private void bindWorkspace() {
            ...
            // Otherwise we hold a reference to them. 获取与Launcher绑定的Callbacks  这是在Launcher中通过app.setLauncher(this);
            ...
            final ArrayList<ItemInfo> tmpWorkspaceItems = unbindWorkspaceItemsOnMainThread(); // 这里还包括AppWidget
            ...
            Collections.sort(tmpWorkspaceItems, new Comparator<ItemInfo>() {
                ...
                public int compare(ItemInfo lhs, ItemInfo rhs) {
                    ...
                    int screenOffset = cellCountX * cellCountY;  // 一个屏幕的cellCount
            ...
            mHandler.post(new Runnable() {
                public void run() {
                    ...
                    if (callbacks != null) {
                        callbacks.startBinding();   // 清空workspace  准备绑定
                    }
                }
            });
            ...
            for (int i=0; i<N; i+=ITEMS_CHUNK) {
                final int start = i;
                final int chunkSize = (i+ITEMS_CHUNK <= N) ? ITEMS_CHUNK : (N-i);   // 每次加载的数目
                mHandler.post(new Runnable() {
                    public void run() {
                        Callbacks callbacks = tryGetCallbacks(oldCallbacks);
                        if (callbacks != null) {
        ...
        private void loadAllAppsByBatch() {
            ...
            while (i < N && !mStopped) {
                ...
                startIndex = i;
                for (int j=0; i<N && j<batchSize; j++) { // 两层循环控制　
                    // This builds the icon bitmaps.　　遍历得到所有的ApplicationInfo（包含图标及title）
                ...
                final boolean first = i <= batchSize;  // frist表示是否是一次性加载
                final Callbacks callbacks = tryGetCallbacks(oldCallbacks);
                final ArrayList<ApplicationInfo> added = mAllAppsList.added; // 所有的ApplicationInfo
                ...
                mHandler.post(new Runnable() {
                    public void run() {
                        ...
                        if (callbacks != null) {
                            if (first) {
                                callbacks.bindAllApplications(added);   // 加载页面实际调用的是Launcher.bindAllApplications()
                            } else {
    ...
    public ShortcutInfo getShortcutInfo(PackageManager manager, Intent intent, Context context,
        ...
        Bitmap icon = null;  // 声明图标及ShortcutInfo
        ...
        Intent newIntent = new Intent(intent.getAction(), null);  // action :android.intent.action.MAIN
        newIntent.addCategory(Intent.CATEGORY_LAUNCHER);
        newIntent.setPackage(oldComponent.getPackageName());  // 构建一个新的Intent
        List<ResolveInfo> infos = manager.queryIntentActivities(newIntent, 0);
        for (ResolveInfo i : infos) {
            ComponentName cn = new ComponentName(i.activityInfo.packageName,
                    i.activityInfo.name);
            if (cn.equals(oldComponent)) {  // 能够查询到
                resolveInfo = i;
            }
        }
        if (resolveInfo == null) {
        ...
        // 取出应用的图标
        if (resolveInfo != null) {
            System.out.println("wbzhang   ----   getApplication Icon");
            icon = mIconCache.getIcon(componentName, resolveInfo, labelCache);
            System.out.println("wbzhang   ----   icon.byte:" + icon.getByteCount());
        }
        // the db
        if (icon == null) {
            if (c != null) {
        ...
        if (icon == null) {
            ...
            info.usingFallbackIcon = true;  // 使用备用图标
        }
        info.setIcon(icon);

        // from the resource  从资源文件获取
        if (resolveInfo != null) {
            ComponentName key = LauncherModel.getComponentNameFromResolveInfo(resolveInfo); // 有可能是一个服务
            if (labelCache != null && labelCache.containsKey(key)) {
                info.title = labelCache.get(key);
            } else {
             // 第一次的时候labelCache没有记录这个key
        ...
        // fall back to the class name of the activity  备用的用ClassName
        if (info.title == null) {
    ...
    boolean queueIconToBeChecked(HashMap<Object, byte[]> cache, ShortcutInfo info, Cursor c,
        ...
        if (!info.customIcon && !info.usingFallbackIcon) {
            cache.put(info, c.getBlob(iconIndex));  // 将图标放入到一个HashMap中
            return true;
        }
        return false;
    }
    ...
    public static class ShortcutNameComparator implements Comparator<ResolveInfo> {
        ...
        public final int compare(ResolveInfo a, ResolveInfo b) {
            // 比较方法　　通过它们的ResolveInfo 获取到它们的Label名再进行排序　　主要是通过Collator这个类
