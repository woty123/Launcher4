...
public class PagedViewCellLayout extends ViewGroup implements Page {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        // Mode :the mode of the measure specification only UNSPECIFIED  EXACTLY    AT_MOST
...
System.out.println("wbzhang ----  mOriginalWidthGap:" + mOriginalWidthGap);
System.out.println("wbzhang ----  mOriginalHeightGap:" + mOriginalHeightGap);
        ...
        if (widthSpecMode == MeasureSpec.AT_MOST) {
            // 计算加上Padding 及Gap得到最后的Width及Height
        ...
        final int count = getChildCount();
        for (int i = 0; i < count; i++) {
            // Child 是一个PagedViewCellLayoutChildren　　这里是给它规定Width and Height
    ...
    public int estimateCellHSpan(int width) {
        ...
        // 因为N个Cell只有N-1个空隙 因此需要在availWidth加上一个mWidthGap得到虚拟的总宽度再去计算
    ...
    public void calculateCellCount(int width, int height, int maxCellCountX, int maxCellCountY) {
        // estimateCellHSpan方法去计算实际CellCountX　　CellCountY是同理的
