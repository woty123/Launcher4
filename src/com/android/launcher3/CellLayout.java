...
public class CellLayout extends ViewGroup {
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
        ...
        // Hotseat icons - remove text　　Hotseat icons　移除text
        ...
        if (lp.cellX >= 0 && lp.cellX <= mCountX - 1 && lp.cellY >= 0 && lp.cellY <= mCountY - 1) {
            ...
            // mean that it spans the extent of the CellLayout　　从它的起点开始随便扩张
            if (lp.cellHSpan < 0) lp.cellHSpan = mCountX;
            if (lp.cellVSpan < 0) lp.cellVSpan = mCountY;

            child.setId(childId);    // 这个ID是根据它的位置生成的，方法：LauncherModel.getCellLayoutChildId(container, screen, x, y, spanX, spanY)
